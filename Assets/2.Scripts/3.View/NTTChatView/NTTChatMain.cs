...
public class NTTChatMain : MonoBehaviour
{
    private IList<KeyValuePair<string, NTTChatMessageObj>> m_MessageObjPool = new List<KeyValuePair<string, NTTChatMessageObj>>();
    ...
    {
        foreach (KeyValuePair<string, NTTChatMessageObj> keyValuePair in m_MessageObjPool)
        {
    ...
    void AddMessageToChat(VivoxMessage message, bool isHistory = false, bool scrollToBottom = false)
    {
        ...
        var newMessageTextObject = newMessageObj.GetComponent<NTTChatMessageObj>();
        if (isHistory)
        {
            m_MessageObjPool.Insert(0, new KeyValuePair<string, NTTChatMessageObj>(message.MessageId, newMessageTextObject));
            newMessageObj.transform.SetSiblingIndex(0);
        }
        else
        {
            m_MessageObjPool.Add(new KeyValuePair<string, NTTChatMessageObj>(message.MessageId, newMessageTextObject));
        }

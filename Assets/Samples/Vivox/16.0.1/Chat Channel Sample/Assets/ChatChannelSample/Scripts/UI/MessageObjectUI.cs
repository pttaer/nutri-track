...
public class MessageObjectUI : MonoBehaviour
{
    ...
    public void SetTextMessage(VivoxMessage message, bool deleted = false)
    {
        ...
        Debug.Log(JsonConvert.SerializeObject(m_vivoxMessage));
        if (deleted)
        {
        ...
        if (message.FromSelf)
        {
            ...
            MessageText.text = string.Format($"<color=blue><size=16>{message.SenderDisplayName}</size> </color>: {message.MessageText}\n<color=#5A5A5A><size=12>{editedText}{message.ReceivedTime}</size></color>");
        }
        else
        {
            MessageText.alignment = TextAnchor.MiddleLeft;
            MessageText.text = string.Format($"<color=green><size=16>{message.SenderDisplayName}</size> </color>: {message.MessageText}\n<color=#5A5A5A><size=12>{editedText}{message.ReceivedTime}</size></color>");
        }
